...
def correlation_vs_model_v3(data,trigger,modelH1,modelH2,modelL):
    ...
    while trigger[it]>-2:#we use the trigger to find the beginning of ML algorithm
        it+=1
    it+=3243#to remove the  part before the ladderstep
    j=it
    n_points=0
    while trigger[j]<-2:
        n_points+=1
    ...
    for k in range(256):
        for j in range(len(modelH1)):
            ...
            seg.append(data[i])
            it+=1
        it+=(n_points/256)-len(modelH1)+0.15#during ML algorithm n_points are captured and there is 256 laddersteps
        seg-=np.mean(seg)
        allseg.append(seg)
        seg=[]
    ...
    #         level.append(1)
    #     elif array_corrH1[i]<array_corrL[i]:
    #         level.append(0)
    #     elif array_corrH2[i]>array_corrL[i]:
    #         level.append(1)
    #     elif array_corrH2[i]<array_corrL[i]:
    #         level.append(0)
    #     elif array_corrH1[i]>array_corrH2[i]:
    #         level.append(1)
    #     elif array_corrH1[i]<array_corrH2[i]:
    #         level.append(1)
    #     else:
